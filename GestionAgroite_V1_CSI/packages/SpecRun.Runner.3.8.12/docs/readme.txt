...
Fixes:
- Fix: Generate temp files with Guid in the name #2294
...
- Fix: Environment variables shared between targets #2132

3.4.51:

Fixes:
- Fix: Report Templates issue - https://github.com/SpecFlowOSS/SpecFlow/issues/2026
- Fix: Testexplorer doesn't show all tests when using tags on examples in scenario outlines
- Fix: SpecRun exe does not display the activation link

3.4.19:

Fixes:
- Fix assembly loading issue - https://github.com/SpecFlowOSS/SpecFlow/issues/2088
...
Changes:
- Add `SpecFlow.Tools.MSBuild.Generation` as a dependency to the `SpecRun.SpecFlow.3-3-0` package

Fixes:
- Fix debugger when using Process test thread isolation
- handle proxy authentication for product activation better
- fix minor issues with product activation
...
Fixes:
- Fix BoDi.ObjectContainerException when retrying a failed test running in parallel - https://github.com/SpecFlowOSS/SpecFlow/issues/1857
- Fix BeforeTestRun hook is executed multiple times when running in parallel- https://github.com/SpecFlowOSS/SpecFlow/issues/1917
- Report: generate empty reports
- Fix console output on Linux and Mac
- Display activation result in the logs
- Executing in Bamboo build agents works
